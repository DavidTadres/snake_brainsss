...
from analysis_scripts import PMB_correlation
...
TIME_BEFORE_TURN = [1,2,3,4,5] # seconds
...
rule PMB_correlation_rule:
    ...
    output:
        savepath_ch1="{corr_imaging_paths}/corr/channel_1_corr_{corr_behavior}_PMB_{time_before_turn}s_no_turn.nii" if 'channel_1' in FUNCTIONAL_CHANNELS else[],
        savepath_ch2="{corr_imaging_paths}/corr/channel_2_corr_{corr_behavior}_PMB_{time_before_turn}s_no_turn.nii" if 'channel_2' in FUNCTIONAL_CHANNELS else[],
        savepath_ch3="{corr_imaging_paths}/corr/channel_3_corr_{corr_behavior}_PMB_{time_before_turn}s_no_turn.nii" if 'channel_3' in FUNCTIONAL_CHANNELS else[]
    run:
        try:
            PMB_correlation.PMB_correlation(
            ...
            print('error: ')
            print(error_stack)
            logfile = utils.create_logfile(fly_log_folder,function_name='ERROR_PMB_correlation')
