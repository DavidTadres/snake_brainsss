...
fictrac_raw = load_fictrac(pathlib.Path(path, 'func_0/fictrac/fictrac_behavior_data.dat'))
timestamps = load_timestamps(pathlib.Path(path, 'func_0/imaging/recording_metadata.xml'))
...
for current_behavior in behaviors:
    if current_behavior == 'dRotLabY':
        short = 'Y'
    elif current_behavior == 'dRotLabZ':
        short = 'Z'
    elif current_behavior == 'heading':
        short = 'h'
    fictrac[short] = smooth_and_interp_fictrac(fictrac_raw, fps, resolution, expt_len, current_behavior, timestamps[:,20])"""

def vectorized_pearson(brain, fictrac):
    """
    Emulates scipy.stats.pearson but instead of doing 1D pearons can do it for our brain volume and is therefore
    much faster.
    Doesn't calculate p-values (which I haven't seen being used in any brainsss code yet anyways)
    :param brain:
    :param fictrac:
    :return:
    """
...
def warp_STA_brain(STA_brain, fly, fixed, anat_to_mean_type):
    ...
    #)
    dataset_path = ('/Volumes/groups/trc/data/David/Bruker/preprocessed/nsybGCaMP_tdTomato')
...
warps_ZPOS = warp_STA_brain(STA_brain=STA_brain, fly='fly_002',fixed=fixed, anat_to_mean_type='myr')
