...
cluster_labels = np.load(
    pathlib.Path(path_fly, "func0/clustering/channel_2_cluster_labels.npy")
)
cluster_signals = np.load(
    pathlib.Path(path_fly, "func0/clustering/channel_2_cluster_signals.npy")
)
...
timestamps = load_timestamps(
    pathlib.Path(path_fly, "func0/imaging/recording_metadata.xml")
)
"""## to vectorize
corrs = []
behavior = 'dRotLabY'
...
fictrac_interp = smooth_and_interp_fictrac(
    fictrac_raw, fps, resolution, expt_len, behavior, timestamps=timestamps
)
...
for z in range(corr_brain.shape[0]):
    ...
    corr_brain[z, :] = np.dot(
        signal_mean_m / normsignal[:, None], fictrac_mean_m / normfictrac
    )
