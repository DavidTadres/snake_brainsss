    ...
    for z in range(brain.shape[2]):
        ...
        for x in range(brain.shape[0]):
            for y in range(brain.shape[1]):
                corr_brain[x, y, z] = pearsonr(
                    fictrac_now[idx_to_use], brain[x, y, z, :][idx_to_use]
                )[0]
...
def fully_vectorized_func():
    ...
    fvect_corr_brain = np.dot(
        brain.reshape(
            brain.shape[0], brain.shape[1], int(brain.shape[2] * brain.shape[3])
        ),
