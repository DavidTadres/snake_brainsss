...
original_fly_path = pathlib.Path('/Volumes/groups/trc/data/Brezovec/2P_Imaging/20190101_walking_dataset/fly_308')
my_fly_paths = pathlib.Path('/Volumes/groups/trc/data/David/Bruker/preprocessed/nsybGCaMP_tdTomato/fly_002')
...
def compare_two_3D_arrays(original_brain_path, my_brain_path, savepath):
    ...
    ax1.imshow(original_mean_brain[:,:,int(round(original_mean_brain.shape[2]/2))].T)
    ax1.set_title(original_fly_path.name + ', mean brain, z=' + repr(int(round(original_mean_brain.shape[2]/2))))
    ax2=fig.add_subplot(222, sharex=ax1, sharey=ax1)
    ax2.imshow(my_mean_brain[:,:,int(round(my_mean_brain.shape[2]/2))].T)
    ax2.set_title(my_fly_paths.name + ', mean brain, z=' + repr(int(round(my_mean_brain.shape[2]/2))))

    # Next plot delta of the two brain
    ax3=fig.add_subplot(223, sharex=ax1, sharey=ax1)
    subtracted_brain = my_mean_brain[:,:,int(round(my_mean_brain.shape[2]/2))].T - original_mean_brain[:,:,int(round(original_mean_brain.shape[2]/2))].T
    ax3.imshow(subtracted_brain)
    ax3.set_title('subtracted brain slice #' + repr(int(round(my_mean_brain.shape[2]/2))))
    ...
    ax4.set_yscale('log')
    delta = original_mean_brain - my_mean_brain # what's the difference in value between the two arrays?
    ax4.set_title('Max abs delta between arrays\n' + repr(round(np.max(np.abs(delta)), 10)))
...
compare_two_3D_arrays(original_brain_path=pathlib.Path(original_fly_path, 'func_0/imaging/functional_channel_1_mean.nii'),
...
compare_two_3D_arrays(original_brain_path=pathlib.Path(original_fly_path, 'func_0/imaging/functional_channel_2_mean.nii'),
...
compare_two_3D_arrays(original_brain_path=pathlib.Path(original_fly_path, 'anat_0/imaging/anatomy_channel_1_mean.nii'),
...
compare_two_3D_arrays(original_brain_path=pathlib.Path(original_fly_path, 'anat_0/imaging/anatomy_channel_2_mean.nii'),
