...
if len(FICTRAC_PATHS) > 0:
    ...
    # If there's no fictrac data, set this to something so it exists.
    # It won't be called.
    fictrac_rel_path_correlation = None
...
rule all:
    ...
    input:
        ...
        #
        expand(str(fly_folder_to_process_oak)
               + "/{moco_meanbr_imaging_paths_misc}/moco/channel_{meanbr_moco_ch_misc}_moco_mean_misc.nii",
               moco_meanbr_imaging_paths_misc=list_of_paths_misc_imaging,
               meanbr_moco_ch_misc=list_of_misc_channels),
        ####
        # Z-score
        ####
        expand(str(fly_folder_to_process_oak)
