...
def get_visually_evoked_turns(
    traces, mean_turn, start, stop, r_thresh, av_thresh, stim_times, expected_direction
):
...
def make_STA_brain(neural_signals, neural_timestamps, event_times_list, neural_bins):
    ...
    for z in range(49):
        ...
        for bin_num in np.arange(1, len(neural_bins)):
            ...
            average_neural_in_bin = np.mean(
                neural_signals[z, :, this_bin_sample_times], axis=0
            )
...
def STA_supervoxel_to_full_res(STA_brain, cluster_labels):
    ...
    for z in range(49):
        ...
        for cluster_num in range(n_clusters):
            ...
            colored_by_betas[:, cluster_indicies] = STA_brain[
                z, :, cluster_num, np.newaxis
            ]
...
def extract_roi_signal_traces(
    ...
    for roi in roi_ids[hemi]:
        mask = roi_masks[roi]
        masked_data = (
            all_warps[condition][:, :, :, ::-1] * mask[np.newaxis, :, :, :]
        )  # note z-flip
        if signal_type == "max":
