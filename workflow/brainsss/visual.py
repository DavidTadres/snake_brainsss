    ...
    for fly_id in fly_ids:
        ...
        for serie in series:
            ...
            for i, epoch_id in enumerate(epoch_ids):
                stim_id = (
                    file["Flies"][fly_id]["epoch_runs"][serie]
                    .get("epochs")
                    .get(epoch_id)
                    .attrs["component_stim_type"]
                )
            ...
            if len(stim_ids) > 100:
                if found_a_full_series:
                    printlog(
                        "WARNING - FOUND 2 FULL SERIES IN THIS VISPROTOCOL HDF5 FILE. YOU NEED TO RESOLVE WHICH TO CHOOSE"
                    )
                    printlog("QUITING")
                    return None
